...
import { PlanChangeDialog } from '@/components/dashboard/subscriptions/components/plan-change-dialog';
...
export function SubscriptionHeader({ subscription }: Props) {
  ...
  const isScheduledToBeCanceled = subscription.scheduledChange && subscription.scheduledChange.action === 'cancel';
  const canBeResumed = isActiveOrTrialing && isScheduledToBeCanceled;

  return (
    <div className={'flex justify-between items-start sm:items-center flex-col sm:flex-row mb-6 sm:mb-0'}>
      <div className={'flex flex-col w-full'}>
        {canBeResumed && <SubscriptionAlerts subscription={subscription} />}
        ...
        <div className={'flex items-center gap-6 py-8 pb-6 flex-wrap md:flex-wrap'}>
          <div className={'flex gap-1 items-end'}>
            ...
            <span className={'text-muted-foreground text-sm leading-[14px] font-medium'}>{frequency}</span>
          </div>
          <div>
            <Status status={subscription.status} />
          </div>
        </div>
        <div className={'text-muted-foreground text-base leading-5 pb-8'}>Started on: {formattedStartedDate}</div>
      </div>
      <div className="flex gap-3 flex-col sm:flex-row">
        {isActiveOrTrialing && !isScheduledToBeCanceled && (
          <PlanChangeDialog
            subscription={subscription}
            trigger={
        ...
        {isActiveOrTrialing && <SubscriptionHeaderActionButton subscription={subscription} />}
      </div>
    </div>
  );
}
