...
export const columns: ColumnDef<Transaction>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      const isReceiptable = ['completed', 'paid', 'billed'].includes((status || '').toLowerCase());
      if (!isReceiptable) return null;
      return (
        <div className="text-right">
          <Link
            className="text-sm underline hover:opacity-80"
            target="_blank"
            href={`/api/payments/invoice?transactionId=${id}&disposition=inline`}
          >
            View receipt
          </Link>
        </div>
