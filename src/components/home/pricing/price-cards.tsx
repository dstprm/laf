...
export function PriceCards({ loading, frequency, priceMap }: Props) {
  ...
  const getButtonHref = (priceId: string) => {
    if (isSignedIn) {
      return `/checkout/${priceId}`;
    }
    // Redirect to login with return URL to checkout
    return `/login?return_url=${encodeURIComponent(`/checkout/${priceId}`)}`;
  };
  ...
  return (
    <div className="isolate mx-auto grid grid-cols-1 gap-8 lg:mx-0 lg:max-w-none lg:grid-cols-3">
      {PricingTier.map((tier) => (
        <div
          key={tier.id}
          className={cn('rounded-lg bg-background/70 backdrop-blur-[6px]rounded-b-none border border-border')}
        >
          <div className={cn('flex gap-5 flex-col rounded-lg ')}>
          ...
          <div className={'px-8 mt-8'}>
            <Button className={'w-full'} variant={'secondary'} asChild={true}>
              <Link href={getButtonHref(tier.priceId[frequency.value])}>Get started</Link>
            </Button>
          </div>
